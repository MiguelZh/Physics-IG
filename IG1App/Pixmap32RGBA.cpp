...
// tipo y valores para nuestro caso: bmp en modo bgr de 24 bits con los pixeles
// por filas bottom to top y columnas left to right
struct bitmap_file_header {
  /*WORD*/ unsigned short signature;  // 'BM'
  /*DWORD*/ unsigned int
      size_file;  // unreliable ->  = (offset_map + size_map) bytes
  /*WORD*/ unsigned short reserved1;  // 0
  /*WORD*/ unsigned short reserved2;  // 0
  /*DWORD*/ unsigned int
      offset_map;  // 54 (14 fileheader + 40 information_header) bytes
};

struct bitmap_information_header {
  /*DWORD*/ unsigned int
      size_info_header;  // 40 bytes = sizeof(bitmap_information_header)
  ...
  /*DWORD*/ unsigned int
      compression;  // 0 = BI_RGB, no pixel array compression used
...
void PixMap32RGBA::load_headers(
    ...
    unsigned int& height) const  // throw(std::ios_base::failure);
{
  ...
  if (bfh.signature != 0x4D42)  // 'BM'
  {
    throw std::ios_base::failure(
        "PixMapRGBA::load_bmpBGR() ERROR: Invalid signature value");
  }
  if (bih.bits_per_pixel != 24)  // 3(BGR) bytes
  {
    throw std::ios_base::failure(
        "PixMapRGBA::load_bmpBGR() ERROR: Invalid depth color (bits per "
        "pixel)");
  }
  if (bih.size_info_header != 40)  // size_of(bitmap_information_header)
  {
    throw std::ios_base::failure(
        "PixMapRGBA::load_bmpBGR() ERROR: Invalid size information header");
  }
  if (bih.clr_used != 0)  // color palette
  {
    throw std::ios_base::failure(
        "PixMapRGBA::load_bmpBGR() ERROR: Invalid number of colors in the "
        "palette");
  }
}
//-------------------------------------------------------------------------------------------
...
void PixMap32RGBA::save_headers(std::ofstream& stream) const  // noexcept;
{
  ...
  bih.size_map =
      ((bih.width * 3) + ((4 - ((3 * width_) % 4)) % 4)) * bih.height;
  bih.x_prpm = 0;  //  Print resolution 2835 = 72 dpi × 39.3701 inches per meter
                   //  yields 2834.6472
  bih.y_prpm = 0;  //  Print resolution 2835 = 72 dpi × 39.3701 inches per meter
                   //  yields 2834.6472
  bih.clr_used = 0;
  bih.clr_important = 0;
...
void PixMap32RGBA::create_pixmap(GLsizei width,
                                 GLsizei height)  // throw(std::bad_alloc)
{
  if (width != width_ || height != height_) {
    ...
    try {
      ...
      throw std::exception(
          "PixMap32RGBA::create_pixmap() ERROR: Could not allocate memory "
          "(bad_alloc)");
    }
    if (data_ == nullptr) {
      width_ = 0;
...
void PixMap32RGBA::load_bmp24BGR(
    const std::string&
        file_name)  // throw(std::ios_base::failure, std::bad_alloc)
{
  std::ifstream stream(file_name.c_str(), std::ios::binary);
  stream.exceptions(std::ifstream::failbit | std::ifstream::badbit);
  if (!stream.is_open()) {
    throw std::ios_base::failure(
        "PixMap32RGBA::load_bmpBGR() ERROR: Could not open file");
  }
...
void PixMap32RGBA::row_rgba_bgr(char* rowAux,
...
void PixMap32RGBA::save_bmp24BGR(
    const std::string& file_name) const  // throw(std::ios_base::failure)
{
  ...
  if (!stream.is_open()) {
    throw std::ios_base::failure(
        "PixMap32RGB::save_bmpBGR() ERROR: Could not open file");
  }
...
PixMap32RGBA::PixMap32RGBA(const unsigned int width,
...
PixMap32RGBA& PixMap32RGBA::operator=(
    const PixMap32RGBA& pixmap)  // throw(std::exception)
{
  if (this != &pixmap) {
